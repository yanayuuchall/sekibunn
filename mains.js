...
document.getElementById("solvedBtn").addEventListener("click", function() {
    if (currentProblem) {
        ...
        problemHistory.push({ number: currentProblem.number, result: "解けた" });
        updateResultDisplay();
    }
});
...
document.getElementById("unsolvedBtn").addEventListener("click", function() {
    if (currentProblem) {
        ...
        problemHistory.push({ number: currentProblem.number, result: "解けなかった" });
        updateResultDisplay();
    }
});
...
function updateResultDisplay() {
    ...
    resultDisplay.innerHTML = `解けた: ${solvedCount} 解けなかった: ${unsolvedCount}`;
    
    const historyDisplay = problemHistory.map(entry => `問題 ${entry.number}: ${entry.result}`).join("<br>");
    resultDisplay.innerHTML += `<br>履歴:<br>${historyDisplay}`;

    const incorrectDisplay = Object.entries(incorrectProblems).map(([number, count]) => `問題 ${number}: ${count} 回間違えた`).join("<br>");
    resultDisplay.innerHTML += `<br>間違えた問題:<br>${incorrectDisplay}`;
}
